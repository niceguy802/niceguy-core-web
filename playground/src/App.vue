...
import { createHttpClient, HttpClient, URL_KEY, createRetryMiddleware, createAuthMiddleware } from '@/framework/http'
import {
  login, getUserInfo, refreshAuthToken, logout,
  getAccessToken, getRefreshToken
} from '@/framework/auth'

// ===== 全局 $url 实例（由 createHttpPlugin 注入，所有中间件已内置） =====
...
async function testFullChain() {
  loading.value = true; result.value = ''
  try {
    result.value = JSON.stringify(await httpFull.get('/posts/1'), null, 2) +
      '\n\n(完整链: Auth → Retry → axios)'
  } catch (e: any) { result.value = 'ERROR: ' + e.message }
  finally { loading.value = false }
}
...
// ---- \ 测试 ----
async function testUrlPost() {
  loading.value = true; result.value = ''
  try {
    result.value = JSON.stringify(await $url.post('/public/auth/login', { username: 'admin', password: '123456' }), null, 2)
    logs.push('[$url] POST 登录成功'); refreshAuthStatus()
  } catch (e: any) { result.value = '$url POST 失败: ' + e.message }
  finally { loading.value = false }
}

async function testUrlGetUser() {
  loading.value = true; result.value = ''
  try { result.value = JSON.stringify(await $url.get('/auth/getUserInfo'), null, 2); logs.push('[$url] GET 用户信息成功') }
  catch (e: any) { result.value = '$url GET 失败: ' + e.message }
  finally { loading.value = false }
}
...
<template>
  <div class="container">
    ...
    <h2>\ 全局实例</h2>
    <p class="hint">createHttpPlugin 注入，所有中间件已内置 — $url.get / $url.post 直接可调用</p>
    <div class="controls">
      <button :disabled="loading" class="btn-primary" @click="testUrlPost">$url POST 登录</button>
      <button :disabled="loading || !authStatus.loggedIn" class="btn-success" @click="testUrlGetUser">$url GET 用户信息</button>
    </div>
...
.hint { font-size: 0.85rem; color: #666; margin: 0 0 0.5rem; }
