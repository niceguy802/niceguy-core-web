...
const getDelay = (delay: RetryOptions['delay'], attempt: number): number => {
  ...
  return async (ctx: HttpContext, next: () => Promise<void>) => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        const delayMs = getDelay(options.delay, attempt)
        console.warn(`[Retry] attempt ${attempt}/${maxRetries} failed, retrying in ${delayMs}ms`, ctx.config.url)

        // 用赋值代替 delete → 避免 V8 对象降级为字典模式
